       ...
       // GET: /System/Message/
        ...
        public JsonResult ChangeMessageState(string paramData)
        {
            ...
            try
            {
                ...
                //修改消息状态
                StringBuilder strSql = new StringBuilder(" Update tblMessage set State=1 where Rid in " + DBContext.DataDecision.AssemblyInCondition(RidArr.ToList()));
        ...
        public ActionResult DataDel()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(paramData))
                {
                    ...
                    StringBuilder strSql = new StringBuilder(" delete from tblMessage where Rid in " + DBContext.AssemblyInCondition(RidArr.ToList()));
                    List<tblMessage> modelList = BusinessContext.tblMessage.GetModelList("Rid in " + DBContext.AssemblyInCondition(RidArr.ToList())).ToList();
