    ...
    public class BusinessRightsController : Controller
    {
        ...
        public ActionResult DataDel()
        {
            ...
            try
            {
                ...
                StringBuilder strSql = new StringBuilder(" delete from sys_role_right where  rf_Type='业务权限' and rf_right_code in  " + DBContext.DataDecision.AssemblyInCondition(RidArr.ToList()));
                lsSql.Add(strSql.ToString());
                strSql = new StringBuilder(" delete from OperationalAuthority where Rid in " + DBContext.AssemblyInCondition(RidArr.ToList()));
                lsSql.Add(strSql.ToString());
                List<OperationalAuthority> modelList = BusinessContext.OperationalAuthority.GetModelList("Rid in " + DBContext.AssemblyInCondition(RidArr.ToList())).ToList();
