...
namespace Bitshare.DataDecision.Controllers.Areas.System
{
    public class ProcessManageController : Controller
    {
        ...
        public void SetDropList()
        {
            ...
            RoleList = BusinessContext.sys_role.GetList().GroupBy(p => p.role_name).Select(p => p.Key).Select(p => new SelectListItem { Text = p, Value = p }).ToList();
        ...
        public ActionResult DataDel()
        {
            ...
            try
            {
                ...
                StringBuilder strSql = new StringBuilder(" delete from tblProcessManage where Rid in " + DBContext.AssemblyInCondition(RidArr.ToList()));

                List<tblProcessManage> modelList = BusinessContext.tblProcessManage.GetModelList("Rid in " + DBContext.AssemblyInCondition(RidArr.ToList())).ToList();
