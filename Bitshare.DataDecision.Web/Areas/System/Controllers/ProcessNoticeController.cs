    ...
    public class ProcessNoticeController : Controller
    {
        ...
        public ActionResult Create(tblProcessNotice tblprocessnotice, string IsContinue = "0")
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    int tblRcdid = BusinessContext.tblProcessNotice.Add(tblprocessnotice);

                    RM.IsSuccess = tblRcdid > 0;

                    if (RM.IsSuccess)
                    {
                        tblprocessnotice.TblRcdId = tblRcdid;
        ...
        public ActionResult DataDel()
        {
            ...
            try
            {
                ...
                StringBuilder strSql = new StringBuilder(" delete from tblProcessNotice where TblRcdId in " + DBContext.AssemblyInCondition(tblRcdidArr.ToList()));

                List<tblProcessNotice> modelList = BusinessContext.tblProcessNotice.GetModelList("TblRcdId in " + DBContext.AssemblyInCondition(tblRcdidArr.ToList())).ToList();
