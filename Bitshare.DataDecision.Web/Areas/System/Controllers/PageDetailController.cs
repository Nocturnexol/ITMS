    ...
    public class PageDetailController : Controller
    {
        ...
        public ActionResult Create(tblPageDetail obj, string IsContinue = "0")
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    int tblRcdid = BusinessContext.tblPageDetail.Add(obj);
        ...
        public ActionResult Edit(tblPageDetail collection)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    tblPageDetail old = BusinessContext.tblPageDetail.GetModel(collection.TblRcdID);
        ...
        public ActionResult DataDel()
        {
            ...
            try
            {
                ...
                // 
                StringBuilder strSql = new StringBuilder(" delete from tblPageDetail where TblRcdId in " + DBContext.DataDecision.AssemblyInCondition(tblRcdidArr.ToList()));
                lsSql.Add(strSql.ToString());
                List<tblPageDetail> P_modelList = BusinessContext.tblPageDetail.GetModelList("TblRcdId in " + DBContext.AssemblyInCondition(tblRcdidArr.ToList())).ToList();

                strSql = new StringBuilder(" delete from tblDetailButton where Detail_NameId in " + DBContext.DataDecision.AssemblyInCondition(tblRcdidArr.ToList()));
                lsSql.Add(strSql.ToString());
                List<tblDetailButton> B_modelList = BusinessContext.tblDetailButton.GetModelList("Detail_NameId in " + DBContext.DataDecision.AssemblyInCondition(tblRcdidArr.ToList())).ToList();
