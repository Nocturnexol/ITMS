...
function poplayerFull(title, url, w, h) {
    return parent.layer.full(poplayer(title, url, w, h));
}
        ...
        $.post(url, data, function (result) {
            if (result.IsSuccess) {
                layer.msg("续约成功请修改发布时间！", 1, function () {
                    window.location.href = "/AdOrder/AdOrder/Details?TitleType=MediaOrder&Rid=" + result.Message;
                });
            } else {
                layer.msg("续约失败!");
            }
        ...
        $.post(url, data, function (result) {
            if (result.IsSuccess) {
                layer.msg("改稿成功！", 1, function () {
                    window.location.href = "/AdOrder/AdOrder/CorrectingAdOrderDetails?TitleType=CorrectingAdOrder&Rid=" + result.Message;
                });
            } else {
                layer.msg("改稿失败!");
            }
    ...
    $.post(url, data, function (result) {
        if (result.IsSuccess) {
            layer.msg("预定成功！", 1, function () {
                window.location.href = "/AdOrder/AdOrder/Details?TitleType=MediaOrder&Rid=" + result.ResultData["Rid"];
            });
        } else {
...
function DispatchingList(url, MainKey, id) {
    ...
    var data = { "paramData": Ridlist, "MainKey": MainKey };
    if (Ridlist != null) {
        $.post(url, data, function (result) {
            if (result.IsSuccess) {
