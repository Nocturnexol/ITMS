...
namespace Bitshare.DataDecision.Common
{
    ...
    public class OperateLogHelper
    {
        ...
        /// 记录添加操作日志
        /// </summary>
        public static void Create<T>(T obj) where T : new()
        {
        ...
        /// 记录批量删除操作日志
        /// </summary>
        public static void Create<T>(List<T> objList) where T : new()
        {
            if (objList.Count > 0)
            {
                StringBuilder sb = new StringBuilder();
        ...
        public static void Edit<T>(T newObj, T oldObj) where T : new()
        {
            StringBuilder sb = new StringBuilder();
            Type type = typeof(T);
        ...
        public static void Edit<T>(List<T> oldList, List<T> newList) where T : new()
        {
            List<sysOperateLog> list = new List<sysOperateLog>();
            for (int i = 0; i < oldList.Count; i++)
            {
                list.Add(EditList<T>(newList[i], oldList[i]));
            }
        ...
        /// 记录删除操作日志
        /// </summary>
        public static void Delete<T>(T obj) where T : new()
        {
            StringBuilder sb = new StringBuilder();
        ...
        /// 记录批量删除操作日志
        /// </summary>
        public static void Delete<T>(List<T> objList) where T : new()
        {
            if (objList.Count > 0)
            {
                StringBuilder sb = new StringBuilder();
